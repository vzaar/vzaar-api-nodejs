...
module.exports = (function(){
  ...
  Api.prototype = {
    ...
    s3Upload: function(path, callback){
      this.signature(function(statusCode, signature){
        if (statusCode === 200) {
          ...
          callback(statusCode, Core.parseJson(signature.data));
        }
      });
    },
    ...
    _linkUpload: function(callback, data){
      var req = new Requests.LinkUpload(this.options);
      req.post(callback, data);
    },

    linkUpload: function(callback, data){
      var that = this;
      this.signature(function(statusCode, signature){
        if (statusCode === 200) {
          that._linkUpload(callback, Core.extend(data, signature));
        } else {
          callback(statusCode, Core.parseJson(signature.data));
        };
      });
    ...
    editVideo: function(videoId, callback, data){
      var req = new Requests.EditVideo(this.options, videoId);
      req.put(callback, data);
    },
